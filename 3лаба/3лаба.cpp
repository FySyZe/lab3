...
bool MyStack::Push(CandyInfo data)
{
	if (!Top)
	{
		Top = new Node;
...
bool MyStack::Pop(CandyInfo& data)
{
	if (!Top) return false;
	Node* temp = Top->prev;
...
void CandyInfo::Out()
{
	cout << "Last candy: " << candy << "\t\t" << "Price: " << price << "\t" << "Sweety: " << sweety << "\t" << "Rating: " << rating << endl << endl;
}
...
bool GetFile(MyStack&, CandyInfo&);
void GetCandy(MyStack&, CandyInfo&);
bool RemoveCandy(MyStack& sMain, CandyInfo& k, string);
		...
		case 1:
			do
			{
				...
				case 1:
					...
					break;
				case 2:
					cout << "Enter the title of the candy: ";
					cin >> title;
					RemoveCandy(S, k, title);
					S.Info();
					break;
				case 3:
					while (S.Pop(k))
...
bool GetFile(MyStack& Stack, CandyInfo& k)
{
	ifstream F("candies.txt");
	if (!F)
	{
		cout << "Cant find file" << endl;
		return false;
	}
	while (F >> k.candy >> k.price >> k.sweety >> k.rating)
	{
		Stack.Push(k);
	}
	F.close();
...
void GetCandy(MyStack& Stack, CandyInfo& k)
{
...
bool RemoveCandy(MyStack& sMain, CandyInfo& k, string title)
{
	MyStack sTemp;
